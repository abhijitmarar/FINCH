...
for j = 1:length(z_h)
    ...
    plot(defocus,radius_spherical_wave,'LineWidth',3);
    hold on
end
...
for j = 1:length(z_h)
    ...
    plot(defocus,radius_plane_wave,'LineWidth',3);
    hold on
end
...
for j = 1:length(z_h)
    ...
    plot(defocus,radius_hologram,'LineWidth',3);
    hold on
end
...
for j = 1:length(z_h)
    for i = 1:length(z_s)
        if (z_s(i) == f_o)
            z_r(i) = double(subs(z_r_focus, [f_SLM,z_H],[f_slm,z_h(j)]));
        else
...
for j = 1:length(z_h)
    ...
    plot(defocus,trans_mag,'LineWidth',3);
    hold on
end
