...
for j = 1:length(z_h)
    for i = 1:length(z_s)
        ...
        F_xx = @(a,b) ((N./(q_sidh(a,b)+(bg/N))).*dq_dxs(a,b).*dq_dxs(a,b));
    ...
    plot(defocus,CRLB_x_SIDH(j,:),'LineWidth',3);
    hold on
end
axis([-10 10 0 50]);
title('CRLB_{xy}');
...
for j = 1: length(z_h)
    for i = 1:length(z_s)
        ...
        F_zz = @(a,b) ((N./(q_sidh(a,b)+(bg/N))).*dq_dzr(a,b).*dq_dzr(a,b).*df_zr(j,i).*df_zr(j,i));
    ...
    plot(defocus,CRLB_z_SIDH(j,:),'LineWidth',3)
    hold on
end
axis([-10 10 0 50]);
title('CRLB_{z}');
