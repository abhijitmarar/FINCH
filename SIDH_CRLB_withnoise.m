...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
alpha = zeros(1,length(z_s));
A = zeros(1,length(z_s));
fisher_x = zeros(1,length(z_s));
CRLB_x = zeros(1,length(z_s));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i = 1:length(z_s)
    alpha(i) = abs(k./(2*z_r(i)));
    A(i) = (1./pi*(r_h(i).^2+2.*(z_r(i)./k).*sin(alpha(i)*r_h(i).^2)));
    q_sidh = @(a,b) A(i).*(1+cos(alpha(i).*(a.^2+b.^2)));
    dq_dxs = @(a,b) 2.*A(i).*trans_mag(i).*alpha(i).*sin(alpha(i).*(a.^2+b.^2)).*a;
    F_xx = @(a,b) ((N./q_sidh(a,b)).*dq_dxs(a,b).*dq_dxs(a,b));
